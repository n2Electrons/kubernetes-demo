...
# Update Prometheus Data Source Configuration in Grafana
set -e
...
if [ -n "$prometheus_pod" ]; then
    ...
    if [[ "$test_result" == *'"status":"success"'* ]]; then
        ...
        echo "Prometheus internal test result: $test_result"
    fi
else
    echo "Could not find Prometheus pod for testing"
fi
...
if curl -s "http://localhost:9090/api/v1/query?query=up" >/dev/null; then
    ...
    echo "Prometheus not accessible locally (port forwarding may need time to establish)"
fi
